...
if ('webkitSpeechRecognition' in window && 'speechSynthesis' in window) {
    ...
    // فعال شدن تشخیص صدا هنگام بارگذاری صفحه
    window.onload = function () {
        speak("Please say the number of players, two, three, or four.");
        document.getElementById("subtitle-container").style.display = "block";
        document.getElementById("subtitle-text").innerText = "Please say the number of players (2, 3, or 4).";
        recognition.start();
    };

    recognition.onstart = function () {
        console.log("Voice recognition started...");
        document.getElementById("subtitle-text").innerText = "Listening...";
    };
    ...
    recognition.onresult = function (event) {
        ...
        document.getElementById("subtitle-text").innerText = `You said: "${command}"`;
        speak(`You said: ${command}`);

        if (!isGameStarted) {
            let numberMatch = command.match(/\d+/);
            if (numberMatch) {
                let numPlayers = parseInt(numberMatch[0]);
                if (numPlayers >= 2 && numPlayers <= 4) {
                    startGameWithVoice(numPlayers);
                    isGameStarted = true;
                } else {
                    speak("Invalid input. Please say two, three, or four.");
                    document.getElementById("subtitle-text").innerText = "Invalid input. Please say 2, 3, or 4.";
                }
            }
        } else {
    ...
    recognition.onerror = function (event) {
        ...
        document.getElementById("subtitle-text").innerText = "Error occurred. Try again.";
        speak("Error occurred. Try again.");
    };
    ...
    alert("Speech recognition or speech synthesis is not supported in this browser.");
}

// تابع برای خواندن متن‌ها
function speak(text) {
    const speech = new SpeechSynthesisUtterance(text);
    speech.lang = 'en-US';
    speech.rate = 1;  // تنظیم سرعت خواندن
    speech.volume = 1;  // میزان بلندی صدا
    window.speechSynthesis.speak(speech);
}

function startGameWithVoice(numPlayers) {
    speak(`Starting game with ${numPlayers} players.`);
    document.getElementById("subtitle-text").innerText = `Starting game with ${numPlayers} players...`;

    let selector = document.querySelector("#player-num");
    selector.value = numPlayers;
    ...
    renderBoard();
    speak("Say 'Roll dice' to start.");
}

function handleGameCommands(command) {
    if (command.includes("roll dice")) {
        ...
        speak("Unknown command. Say 'Roll dice', 'Restart', or 'Go home'.");
        document.getElementById("subtitle-text").innerText = "Unknown command. Say 'Roll dice', 'Restart', or 'Go home'.";
    }
}

function rollDiceWithVoice() {
    let result = randIntv1(6) + 1;
    document.getElementById("dice-results").innerText = `Player ${currentPlayer.idx + 1} rolled a ${result}`;
    document.getElementById("subtitle-text").innerText = `Rolling dice... You got ${result}`;
    speak(`Rolling dice. You got ${result}`);
    ...
    currentPlayer = playerIterator.next().value;
    document.getElementById("dice-results").innerText = `Player ${currentPlayer.idx + 1}'s turn`;
    document.getElementById("subtitle-text").innerText = `Say 'Roll dice' to continue.`;
    speak(`Player ${currentPlayer.idx + 1}'s turn. Say 'Roll dice'.`);
}

function restartGame() {
    restart();
    speak("Game restarted. Please say the number of players.");
    document.getElementById("subtitle-text").innerText = "Game restarted. Please say the number of players.";
    isGameStarted = false;
}

function goHome() {
	speak("Returning to home page...");
    document.getElementById("subtitle-text").innerText = "Returning to home page...";
    
    // تغییر مسیر به صفحه Home
